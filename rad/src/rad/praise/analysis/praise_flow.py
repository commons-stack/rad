...
def run(praise_distribution_data, _config={"num-senders":15, "num-receivers":25}):
    """
    ...
    n_senders = _config["num-senders"]
    n_receivers = _config["num-receivers"]



    dataframe_in = pd.DataFrame(praise_distribution_data.praiseInstance.dataTable)
    ...
    df_to = reference_df.groupby(["TO"]).sum(numeric_only = True).copy()
...
def printDescription(praise_distribution_data, _config={"num-senders":15, "num-receivers":25}):
    """
    ...
    # header = f"# Praise flow"
    description = f"This graph visualizes the top 'praise flows' from '{name}'. Thanks to @inventandchill for his awesome work! On one side we have the top {n_senders} praise givers separately, on the other the top {n_receivers} receivers. The people outside the selection get aggregated into the 'REST FROM' and 'REST TO  categories."

    # TODO Jury still out on if we want to completely remove the header, or just separate it into its own method
    # display(Markdown(header))
    display(Markdown(description))


def printGraph(praise_distribution_data, _config={"num-senders":15, "num-receivers":25}):
    """
    ...
    distribution = run(praise_distribution_data, _config)
